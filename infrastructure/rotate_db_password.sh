...
#
# The live production password is FOUR CHARACTERS. It is the `odoo` default that
# docker-compose.yml published as `${DB_PASSWORD:-odoo}` in a PUBLIC repository,
# and it is in git history forever. Deleting the line is not a fix; only changing
# the credential is.
#
...
# ── ORDERING ──────────────────────────────────────────────────────────────────
#
# This CANNOT run before the Phase 4b window. It reads the secret from the VM,
# which needs both roles/secretmanager.secretAccessor on the attached service
# account and the cloud-platform scope on the instance. HRMS has neither today:
# the attached account is the compute default with ZERO project roles, and the
# scopes are the restricted defaults.
#
# ── THE WINDOW ────────────────────────────────────────────────────────────────
#
# Existing pooled connections survive ALTER ROLE; new ones fail until Odoo
# restarts. Kept to seconds by doing both back to back here.

set -euo pipefail

OLD_PW="${1:?usage: rotate_db_password.sh <current-password>}"

# Resolved rather than pinned, so this works on both sides of the Phase 4c move
# out of /home/tech. Same reasoning as scripts/deploy.sh.
if [[ -n "${APP_DIR:-}" ]]; then
  :
  ...
  echo "rotate: FATAL: no checkout at /opt/odoo-hrms or /home/tech/odoo-project" >&2
  exit 1
fi

# HRMS pins container_name in docker-compose.yml, so these are fixed rather than
# compose-generated (the CRM instance has odoo-project-odoo-1 etc).
DB_CONTAINER="${DB_CONTAINER:-odoo-db}"
ODOO_CONTAINER="${ODOO_CONTAINER:-odoo-app}"
SECRET="${SECRET:-odoo-db-password}"
DB_USER="${DB_USER:-odoo}"
DB_NAME="${DB_NAME:-odoo_hrms_db}"
...
NEW_PW="$(gcloud secrets versions access latest --secret="$SECRET" 2>/dev/null)" \
  || die "cannot read secret $SECRET — needs secretAccessor AND the cloud-platform scope (Phase 4b)"
...
# the database container proves nothing — on the CRM instance it passed with a
# deliberately wrong password during testing. Only a connection from another
# host on the network exercises scram-sha-256, which is the path Odoo uses.
